...
from .models import TRANSACTION_MODES, CurrentForm, Company, PlayerStats, ScoreCard, home_team, away_team
...
batters = [(0,'None')]
batters = batters + [tuple([player.id,player.name]) for player in batting_team ]
bowlers = [(0,'None')]
bowlers = bowlers + [tuple([player.id,player.name]) for player in bowling_team ]

team_names = [('Royal Challengers Bangalore', 'Royal Challengers Bangalore'),('Chennai Super Kings', 'Chennai Super Kings'),('Delhi Capitals', 'Delhi Capitals'),('Kings XI Punjab','Kings XI Punjab'),('Kolkata Knight Riders','Kolkata Knight Riders'),('Mumbai Indians','Mumbai Indians'),('Rajasthan Royals','Rajasthan Royals'),('Sunrisers Hyderabad','Sunrisers Hyderabad')]
...
class MatchCreationForm(forms.Form):
    ...
    home_team_players = forms.MultipleChoiceField(label='Home team players', widget=forms.CheckboxSelectMultiple, choices=batters) # This needs to be a dependent drop down based on the home_team
    away_team_players = forms.MultipleChoiceField(label='Away team players', widget=forms.CheckboxSelectMultiple, choices=bowlers) # This needs to be a dependent drop down based on the away_team
